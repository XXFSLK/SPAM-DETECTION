            ...
            if is_prob:
                ...
                if not is_multi:
                    ...
                    for j in range(0, top_k):
                        print(f"predict_label_idx[{j}] = {predict_label_idx[j]}, prob_np.shape = {prob_np.shape}")
...
"""`classification_evaluate.py` 文件定义了一个名为 `ClassificationEvaluator` 的类，它提供了文本分类模型评估的功能。以下是该类的核心内容和功能：

1. **初始化 (`__init__`)**:
   - 构造函数接收一个 `eval_dir` 参数，用于指定评估结果的保存目录。如果目录不存在，则会创建该目录。

2. **计算性能指标 (`_calculate_prf`)**:
   - 一个静态方法，用于计算精确度（precision）、召回率（recall）和F1分数（fscore）。

3. **标签判断 (`_judge_label_in`)**:
   - 一个私有方法，用于判断给定的标签是否在标签映射中。

4. **层级性能计算 (`calculate_level_performance`)**:
   - 计算不同层级的性能指标，包括宏平均（macro average）和微平均（micro average）。

5. **评估 (`evaluate`)**:
   - 核心方法，用于评估预测结果。它接收预测概率或文本标签，并计算混淆矩阵、精确度、召回率和F1分数。
   - 参数包括：
     - `predicts`: 预测概率或预测文本标签。
     - `standard_label_names` 或 `standard_label_ids`: 真实标签名称或ID。
     - `label_map`: 标签映射字典。
     - `threshold`: 用于过滤概率的阈值。
     - `top_k`: 用于多标签评估的参数。
     - `is_prob`: 预测值是否为概率列表。
     - `is_flat`: 是否仅计算平面结果（非层级结果）。
     - `is_multi`: 是否为多标签评估。
     - `other_text`: 用于排除某些标签的文本。

6. **保存混淆矩阵 (`save_confusion_matrix`)**:
   - 一个静态方法，用于将混淆矩阵保存到文件。
...
8. **保存评估结果 (`save`)**:
   - 将最新的评估结果保存到 `eval_dir` 指定的目录中。

这个类的主要作用是为文本分类模型提供一个评估框架，包括计算各种性能指标和保存评估结果。它支持多标签和层级标签的评估，并允许用户根据需要保存详细的评估数据。
"""
