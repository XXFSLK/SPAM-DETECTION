...
class HMCN(Classifier):
    ...
    def __init__(self, dataset, config):
        ...
        for i in range(1, len(self.hierarchical_depth)):  # Avoid the last index change here
    ...
    def forward(self, batch):
        ...
        for i, (local_layer, global_layer) in enumerate(zip(self.local_layers, self.global_layers)):
            ...
            if i < len(self.global_layers)-1:
                # print the shapes before the concatenation step to debug
                print(f"doc_embedding shape: {doc_embedding.shape}")
                print(f"local_layer_activation shape: {local_layer_activation.shape}")
                global_layer_activation = torch.cat((local_layer_activation, doc_embedding), 1)
                print(f"global_layer_activation shape: {global_layer_activation.shape}")
            else:
