...
class DatasetBase(torch.utils.data.dataset.Dataset):
    ...
    def __init__(self, config, json_files, generate_dict=False,
        ...
        for i, json_file in enumerate(json_files):
            with open(json_file,"r", encoding="utf-8", errors="ignore") as fin:     #change here
                self.sample_index.append([i, 0])
                while True:
                    json_str = fin.readline() #change here
                    if not json_str:
        ...
        def _insert_vocab(files, _mode=InsertVocabMode.ALL):
            for _i, _json_file in enumerate(files):
                with open(_json_file,"r", encoding="utf-8", errors="ignore") as _fin:  #change here
    ...
    def __getitem__(self, idx):
        ...
        with open(self.files[index[0]],"r", encoding="utf-8", errors="ignore") as fin:
            fin.seek(index[1])
